...
void Pendulum::initializeBalls() {
    initializeBall(tipBall, config::pendulum::dimensions::ballRadius,
                   config::colors::ballColor, config::pendulum::dimensions::ballOutlineThickness,
                   config::colors::ballOutlineColor,
                   bar.getPosition().x, bar.getPosition().y + bar.getSize().y);

    initializeBall(baseBall, config::pendulum::dimensions::ballRadius, config::colors::ballColor,
                   config::pendulum::dimensions::ballOutlineThickness, config::colors::ballOutlineColor,
...
void Pendulum::initializeTrack(const Vector2f &startPosition) {
    ...
    track.setFillColor(config::colors::trackFillColor);
    track.setOutlineThickness(config::pendulum::dimensions::trackOutlineThickness);
    track.setOutlineColor(config::colors::trackOutlineColor);
...
void Pendulum::initializeBar() {
    ...
    bar.setFillColor(config::colors::barColor);
